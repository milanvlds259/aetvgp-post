...
@export var pan_distance = 200
@export var pan_duration = 10
...
func _ready():
	...
	bg_tween.set_loops()
	bg_tween.tween_property($Titleback, "position:x", $Titleback.position.x + pan_distance, pan_duration)
	bg_tween.tween_property($Titleback, "position:x", $Titleback.position.x, pan_duration)
